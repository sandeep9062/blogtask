...
// We will use a regular function here instead of a `server component` or `use client` component to fix the `params.slug` issue as a Promise, and it is a good practice for data fetching.
export default function BlogDetailPage({ params }: { params: { slug: string } }) {
  const { slug } = params;

  // Since `params` is a Promise in future Next.js, we should handle `slug` as a string directly.
  const {
    data: blog,
    isLoading,
    isError,

...
export default function BlogFormPage() {
  ...
  const normSingleFile = (e: { fileList: UploadFile[] }) => {
    if (Array.isArray(e)) {
      return e;
    }
  ...
  const onFinish = async (values: BlogFormValues) => {
    ...
    Object.keys(values).forEach((key) => {
      ...
      if (key === "image" || key === "authorImage") {
        ...
        if (key === "date" && dayjs.isDayjs(value)) {
    ...
    try {
      ...
      const errorMessage = error.data?.message || "Failed to create blog.";
